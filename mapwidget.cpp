    ...
    for (const auto &pb : data) {
        ...
        if (pb.etat.toUpper() == "INSTALLE" || pb.etat.toUpper() == "INSTALLED") {
            if (isFunctional) {
                ...
                color = "#999999"; // Gris par defaut (Inconnu)
            }
        }
        ...
        QString popupColor = color;
        QString binEmoji = (color == "#4caf50") ? "🟢" : (color == "#f44336" ? "🔴" : "⚪");

        markers += QString(
                       "const marker%1 = L.marker([%2, %3], {\n"
                       "    icon: L.divIcon({\n"
                       "        className: 'custom-div-icon',\n"
                       "        html: '<div style=\"font-size: 24px; display: flex; align-items: center; justify-content: center; width: 34px; height: 34px; background: %4; border-radius: 50%; border: 2px solid white; box-shadow: 0 2px 5px rgba(0,0,0,0.3);\">🗑️</div>',\n"
                       "        iconSize: [34, 34],\n"
                       "        iconAnchor: [17, 17]\n"
                       "    })\n"
                       "}).bindPopup(`\n"
